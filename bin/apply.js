#!/usr/bin/env node
// Copy repo config into live dirs (repo -> ~/.claude). Use --dry-run to preview.
'use strict';
...
function copyEntry(src, dst) {
  ...
  if (sameFile(src, dst)) { console.log(`  same: ${dst} (linked) — skip`); return; }
  if (dryRun) { console.log(`  copy: ${src} -> ${dst}`); return; }
...
for (const [agentName, agent] of Object.entries(manifest.agents)) {
  ...
  if (agent.instructions) {
    copyEntry(path.join(repoRoot, agent.instructions.from), path.join(targetDir, agent.instructions.to));
  }
  for (const entry of agent.sync) {
    copyEntry(path.join(repoDir, entry), path.join(targetDir, entry));
  }
}
